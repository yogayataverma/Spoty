            ...
            <div className='space-y-4'>
                ...
                <div className='flex flex-wrap gap-4'>
                    {artists.slice(0, 4).map((artist) => {
                        return <div onClick={() => selectArtist(artist)} key={artist.id} className='cursor-pointer relative group w-56 mb-2 bg-neutral-800 hover:bg-neutral-600 rounded-full p-4'>
                            ...
                            <p className='text-base text-white mb-1 w-48 truncate ml-10'>{artist.name}</p>
                            <p className='text-sm text-neutral-400 mb-8 w-48 truncate ml-10'>Artist</p>
                        </div>
                    })}
                </div>
            </div>
            <div className='space-y-4 mb-48'>
                ...
                <div className='flex flex-wrap gap-4'>
                    {playlists.slice(0, 4).map((playlist) => {
                        return <div onClick={() => selectPlaylist(playlist)} key={playlist.id} className='cursor-pointer relative group w-56 mb-2 bg-neutral-800 hover:bg-neutral-600 rounded-full p-4'>
                            <div className='absolute opacity-0 group-hover:opacity-100 transition-all ease-in-out duration-200 shadow-2xl shadow-neutral-900 z-10 h-12 w-12 flex items-center justify-center rounded-full bg-green-500 top-[156px] group-hover:top-[148px] right-6'>
                                <PlayIcon className='h-6 w-6 text-black' />
                            </div>
                            <img className='w-48 h-48 mb-4 rounded-full' src={playlist.images[0].url} />
                            <p className='text-base text-white mb-1 w-37 truncate ml-10'>{playlist.name}</p>
                            <p className='text-sm text-neutral-400 mb-8 w-37 truncate ml-10'>By {playlist.owner.display_name}</p>
                        </div>
                    })}
                </div>
            </div>
