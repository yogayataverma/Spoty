...
export default function PlaylistView({globalPlaylistId ,  setGlobalCurrentSongId , setGlobalIsTrackPlaying}) {
      ...
      <div className='flex-grow h-screen w-full'>
        ...
        <div onScroll={(e) => changeOpacity(e.target.scrollTop)} className='relative -top-20 h-screen overflow-y-scroll bg-neutral-900 '>
                <section className={`flex items-end space-x-7 bg-gradient-to-b to-neutral-900 h-80 text-white p-8`}>
                    {playlistData && <img className='h-44 w-44 rounded-full' src={playlistData.images[0].url} />}
                    <div>
