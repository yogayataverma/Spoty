        ...
        <div className='flex flex-col gap-4 px-8 h-screen overflow-y-scroll'>
            ...
            <div className='flex flex-wrap gap-6 mb-48'>
                {playlists.map((playlist) => {
                    return <div onClick={() => selectPlaylist(playlist)} key={playlist.id} className='rounded-full cursor-pointer relative group w-56 mb-2 bg-neutral-800 hover:bg-neutral-600  p-4'>
                        ...
                        <p className='text-base text-white mb-1 w-48 truncate ml-10'>{playlist.name}</p>
                        <p className='text-sm text-neutral-400 mb-8 w-48 truncate ml-10'>By {playlist.owner.display_name}</p>
                    </div>
                })}
            </div>
        </div>
